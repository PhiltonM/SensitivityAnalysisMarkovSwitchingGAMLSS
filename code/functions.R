...
# Function to fit Markov-switching GAMLSS using the msgamboostLSS algorithm
FitMarkovSwitchingGAMLSS <- function(x, y, N = 2, type = "MSGAMLSS", stat = FALSE, m.stop = rep(480, 2), max.iter = 50, conv.tol = 1e-02, conv.print = TRUE, init_state_trans_prob = NULL, formula = NULL) {
  delta <- NULL
  gamma <- if (is.null(init_state_trans_prob)) {
    matrix(c(0.5, 0.5, 0.5, 0.5), ncol = 2)
  } else {
    as.matrix(init_state_trans_prob)
  }
...
cv_msgamlss <- function(x, y, states, init_probs_list, max_iter = 10, mstop = c(100, 100), type = "MSGLMLSS", formula = NULL) {
  ...
  for (init_probs in init_probs_list) {
    ...
    likelihood_list[[toString(init_probs)]] <- likelihood
  }
...
# Function to process cross-validation results
cv_results <- function(results, init_state_probs_list, true_gamma, k) {
  ...
  results_df <- data.frame(
    ...
    likelihood = unlist(results$likelihood)
  )
