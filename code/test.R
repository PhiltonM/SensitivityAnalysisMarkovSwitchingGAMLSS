...
cv_linear <- cv_msgamlss(x = x, y=y, init_probs_list = init_state_probs_list, formula = formula, states = states)


cv_res_linear <- cv_results(cv_linear, init_state_probs_list, true_gamma, k)
gamma_df <- cv_res_linear$gamma_df
results_df <- cv_res_linear$results_df
agg_results_df <- cv_res_linear$agg_results_df
